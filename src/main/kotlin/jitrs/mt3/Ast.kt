...
fun stmtFromSExpr(e: MT3SExpr): Stmt {
    if (e is MT3Node && e.subexprs[0] is MT3Leaf)
        when (e.subexprs[0].cast<MT3Leaf>().token.getIdent()) {
            "let" -> return Stmt.VariableDefinition(e.subexprs[1].cast<MT3Leaf>().token.getIdent(),
...
fun collectFunctionsVariables(
    func: Toplevel.Fun,
    preDeclaredVariables: HashSet<String>
): Array<Stmt.VariableDefinition> {
    @Suppress("UnnecessaryVariable")
    val variableNames = preDeclaredVariables
    ...
    fun visit(stmt: Stmt) {
        when (stmt) {
            is Stmt.VariableDefinition -> {
                if (variableNames.contains(stmt.name))
